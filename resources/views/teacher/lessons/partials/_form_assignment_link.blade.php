...
<div class="space-y-4">
    <div>
        <label for="assignment_id_{{ $lessonType }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Select Assignment</label>
        ...
        @enderror
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Only assignments belonging to this course can be linked.</p>
    </div>

    <div>
        <label for="instructions_assignment_link_{{ $lessonType }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Instructions (Optional)</label>
        <textarea name="instructions" id="instructions_assignment_link_{{ $lessonType }}" rows="5" placeholder="Enter any specific instructions for this linked assignment..."
                  class="rich-text-editor mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-indigo-500 focus:border-indigo-500">{{ old('instructions', $lesson->instructions ?? '') }}</textarea>
        @error('instructions')
        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        @enderror
    </div>
